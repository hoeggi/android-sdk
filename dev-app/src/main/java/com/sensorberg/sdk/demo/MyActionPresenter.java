    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (action.getType()) {
            case MESSAGE_URI:
                ...
                showNotification(context, action.getUuid().hashCode(), uriMessageAction.getTitle(), uriMessageAction.getContent() + delayString, Uri.parse(uriMessageAction.getUri()), action);
                break;
            case MESSAGE_WEBSITE:
                VisitWebsiteAction visitWebsiteAction = (VisitWebsiteAction) action;
                showNotification(context, action.getUuid().hashCode(), visitWebsiteAction.getSubject(), visitWebsiteAction.getBody() + delayString, visitWebsiteAction.getUri(), action);
                break;
            case MESSAGE_IN_APP:
                InAppAction inAppAction = (InAppAction) action;
                showNotification(context, action.getUuid().hashCode(), inAppAction.getSubject(), inAppAction.getBody() + delayString, inAppAction.getUri(), action);
                break;
        }
    }
